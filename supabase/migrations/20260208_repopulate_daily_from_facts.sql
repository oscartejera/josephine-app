...
=======
-- UPGRADE seed_demo_labour_data RPC to populate ALL 7 daily tables
-- ====================================================================
CREATE OR REPLACE FUNCTION public.seed_demo_labour_data(
  p_days int DEFAULT 30, p_locations int DEFAULT 6
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_group_id uuid; v_location RECORD; v_day date;
  v_base_sales numeric; v_sales_multiplier numeric;
  v_net_sales numeric; v_gross_sales numeric; v_orders numeric;
  v_labor_hours numeric; v_labor_cost numeric; v_blended_rate numeric;
  v_forecast_sales numeric; v_forecast_orders numeric;
  v_planned_hours numeric; v_planned_cost numeric;
  v_dow int; v_locations_count int := 0; v_days_count int := 0;
BEGIN
  v_group_id := get_user_group_id();
>>>>>>> ae20bf1 (fix: migrate all frontend queries from empty tickets/timesheets to daily tables)
...
=======
  -- Cleanup existing data for this group
  DELETE FROM pos_daily_metrics WHERE location_id IN (SELECT id FROM locations WHERE group_id = v_group_id);
  DELETE FROM forecast_daily_metrics WHERE location_id IN (SELECT id FROM locations WHERE group_id = v_group_id);
  DELETE FROM pos_daily_finance WHERE location_id IN (SELECT id FROM locations WHERE group_id = v_group_id);
  DELETE FROM labour_daily WHERE location_id IN (SELECT id FROM locations WHERE group_id = v_group_id);
  DELETE FROM cogs_daily WHERE location_id IN (SELECT id FROM locations WHERE group_id = v_group_id);
  DELETE FROM budgets_daily WHERE location_id IN (SELECT id FROM locations WHERE group_id = v_group_id);
  DELETE FROM cash_counts_daily WHERE location_id IN (SELECT id FROM locations WHERE group_id = v_group_id);

  FOR v_location IN SELECT id, name FROM locations WHERE group_id = v_group_id LOOP
    v_locations_count := v_locations_count + 1;
    v_base_sales := CASE
      WHEN v_location.name ILIKE '%HQ%' THEN 18000 + random() * 5000
      WHEN v_location.name ILIKE '%Centro%' OR v_location.name ILIKE '%Central%' THEN 22000 + random() * 8000
      ELSE 8000 + random() * 12000 END;
    v_blended_rate := 12 + random() * 8;

    FOR v_day IN SELECT generate_series(CURRENT_DATE - (p_days || ' days')::interval, CURRENT_DATE - interval '1 day', '1 day')::date LOOP
      v_dow := EXTRACT(DOW FROM v_day);
      v_sales_multiplier := CASE v_dow WHEN 5 THEN 1.35+random()*0.15 WHEN 6 THEN 1.45+random()*0.20
        WHEN 0 THEN 1.10+random()*0.15 WHEN 1 THEN 0.75+random()*0.10 ELSE 0.90+random()*0.15 END;
>>>>>>> ae20bf1 (fix: migrate all frontend queries from empty tickets/timesheets to daily tables)
...
=======
      v_forecast_sales := ROUND((v_net_sales * (0.88 + random() * 0.24))::numeric, 2);
      v_forecast_orders := ROUND((v_orders * (0.90 + random() * 0.20))::numeric, 0);
      v_planned_hours := ROUND((v_labor_hours * (0.85 + random() * 0.30))::numeric, 1);
      v_planned_cost := ROUND((v_planned_hours * v_blended_rate)::numeric, 2);

      INSERT INTO pos_daily_metrics (date,location_id,net_sales,orders,labor_hours,labor_cost)
        VALUES (v_day,v_location.id,v_net_sales,v_orders,v_labor_hours,v_labor_cost)
        ON CONFLICT (date,location_id) DO UPDATE SET net_sales=EXCLUDED.net_sales,orders=EXCLUDED.orders,labor_hours=EXCLUDED.labor_hours,labor_cost=EXCLUDED.labor_cost;
      INSERT INTO forecast_daily_metrics (date,location_id,forecast_sales,forecast_orders,planned_labor_hours,planned_labor_cost)
        VALUES (v_day,v_location.id,v_forecast_sales,v_forecast_orders,v_planned_hours,v_planned_cost)
        ON CONFLICT (date,location_id) DO UPDATE SET forecast_sales=EXCLUDED.forecast_sales,forecast_orders=EXCLUDED.forecast_orders,planned_labor_hours=EXCLUDED.planned_labor_hours,planned_labor_cost=EXCLUDED.planned_labor_cost;
      INSERT INTO pos_daily_finance (date,location_id,net_sales,gross_sales,orders_count,payments_cash,payments_card,payments_other,refunds_amount,refunds_count,discounts_amount,comps_amount,voids_amount)
        VALUES (v_day,v_location.id,v_net_sales,v_gross_sales,v_orders,ROUND((v_net_sales*0.25)::numeric,2),ROUND((v_net_sales*0.70)::numeric,2),ROUND((v_net_sales*0.05)::numeric,2),ROUND((v_net_sales*0.005)::numeric,2),GREATEST(1,FLOOR(v_orders*0.02)),ROUND((v_net_sales*0.03)::numeric,2),ROUND((v_net_sales*0.01)::numeric,2),ROUND((v_net_sales*0.008)::numeric,2))
        ON CONFLICT (date,location_id) DO UPDATE SET net_sales=EXCLUDED.net_sales,gross_sales=EXCLUDED.gross_sales,orders_count=EXCLUDED.orders_count,payments_cash=EXCLUDED.payments_cash,payments_card=EXCLUDED.payments_card,payments_other=EXCLUDED.payments_other,refunds_amount=EXCLUDED.refunds_amount,refunds_count=EXCLUDED.refunds_count,discounts_amount=EXCLUDED.discounts_amount,comps_amount=EXCLUDED.comps_amount,voids_amount=EXCLUDED.voids_amount;
      INSERT INTO labour_daily (date,location_id,labour_cost,labour_hours)
        VALUES (v_day,v_location.id,v_labor_cost,v_labor_hours)
        ON CONFLICT (date,location_id) DO UPDATE SET labour_cost=EXCLUDED.labour_cost,labour_hours=EXCLUDED.labour_hours;
      INSERT INTO cogs_daily (date,location_id,cogs_amount)
        VALUES (v_day,v_location.id,ROUND((v_net_sales*0.28)::numeric,2))
        ON CONFLICT (date,location_id) DO UPDATE SET cogs_amount=EXCLUDED.cogs_amount;
      INSERT INTO budgets_daily (date,location_id,budget_sales,budget_labour,budget_cogs)
        VALUES (v_day,v_location.id,ROUND((v_net_sales*(1.0+0.03*SIN(EXTRACT(DOY FROM v_day))))::numeric,2),ROUND((v_labor_cost*(1.0-0.02*COS(EXTRACT(DOY FROM v_day))))::numeric,2),ROUND((v_net_sales*0.28*(1.0+0.02*SIN(EXTRACT(DOY FROM v_day)+1)))::numeric,2))
        ON CONFLICT (date,location_id) DO UPDATE SET budget_sales=EXCLUDED.budget_sales,budget_labour=EXCLUDED.budget_labour,budget_cogs=EXCLUDED.budget_cogs;
      INSERT INTO cash_counts_daily (date,location_id,cash_counted)
        VALUES (v_day,v_location.id,ROUND((v_net_sales*0.25*(1.0+0.01*SIN(EXTRACT(DOY FROM v_day)*3)))::numeric,2))
        ON CONFLICT (date,location_id) DO UPDATE SET cash_counted=EXCLUDED.cash_counted;
>>>>>>> ae20bf1 (fix: migrate all frontend queries from empty tickets/timesheets to daily tables)
...
=======
  RETURN jsonb_build_object('seeded',true,'locations',v_locations_count,'days',v_days_count/GREATEST(v_locations_count,1));
>>>>>>> ae20bf1 (fix: migrate all frontend queries from empty tickets/timesheets to daily tables)
END;
$$;
