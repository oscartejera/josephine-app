...
export function useInstantPLData({
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Fetch actual sales from pos_daily_finance (aggregated from facts_sales_15m)
      const { data: financeData } = await supabase
        .from('pos_daily_finance')
        .select('location_id, net_sales, gross_sales')
      ...
      const locationMetrics: LocationPLMetrics[] = locations.map(loc => {
        // Actuals from daily finance data
        const locFinance = (financeData || []).filter(f => f.location_id === loc.id);
        const salesActual = locFinance.reduce((sum, f) => sum + (Number(f.net_sales) || Number(f.gross_sales) || 0), 0);
