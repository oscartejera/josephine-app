...
def fetch_daily_sales(supabase: SupabaseREST, location_id: str) -> pd.DataFrame:
    """Fetch daily sales from product_sales_daily (aggregated, clean data).

    Falls back to tickets if product_sales_daily is empty.
    """
    # Primary source: product_sales_daily (pre-aggregated, always clean)
    all_rows = []
    page_size = 1000
    offset = 0
    ...
    if all_rows:
        ...
        df["y"] = df["net_sales"].astype(float)
    ...
    # Fallback: tickets table
    logger.info("product_sales_daily empty, falling back to tickets...")
    all_tickets = []
    offset = 0
...
HYPERPARAMETER_GRID = [
    # Additive: best for data with absolute-scale noise
    {"changepoint_prior_scale": 0.005, "seasonality_prior_scale": 5, "holidays_prior_scale": 5,
     "seasonality_mode": "additive"},
    {"changepoint_prior_scale": 0.01, "seasonality_prior_scale": 10, "holidays_prior_scale": 10,
     "seasonality_mode": "additive"},
    {"changepoint_prior_scale": 0.05, "seasonality_prior_scale": 10, "holidays_prior_scale": 10,
     "seasonality_mode": "additive"},
    # Multiplicative: best for data with percentage-scale noise
    {"changepoint_prior_scale": 0.005, "seasonality_prior_scale": 5, "holidays_prior_scale": 5,
...
def _build_model(
    ...
    mode = params.get("seasonality_mode", "multiplicative")
    m.add_seasonality(name="monthly", period=30.5, fourier_order=5)

    # Custom regressors
    m.add_regressor("is_payday", prior_scale=2, mode=mode)
    m.add_regressor("temperature", prior_scale=3, standardize=True, mode=mode)
    m.add_regressor("is_rainy", prior_scale=5, mode=mode)
    m.add_regressor("is_cold", prior_scale=3, mode=mode)
    m.add_regressor("is_hot", prior_scale=2, mode=mode)
...
def train_and_evaluate(
    ...
    if n_days >= 365:
        initial_days = 180  # 6 months initial
        horizon_days = 14   # 2-week forecast horizon for CV
        period_days = 14
    elif n_days >= 120:
